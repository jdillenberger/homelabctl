...
func (m *Manager) ensureNetwork() error {
	...
	_, err := m.runner.Run("sh", "-c",
		fmt.Sprintf("docker network create %s 2>/dev/null || true", network))
	if err != nil {
...
func (m *Manager) Deploy(appName string, opts DeployOptions) error {
	...
	infoData, _ := json.MarshalIndent(deployed, "", "  ")
	if err := os.WriteFile(filepath.Join(appDir, ".homelabctl.json"), infoData, 0o644); err != nil {
